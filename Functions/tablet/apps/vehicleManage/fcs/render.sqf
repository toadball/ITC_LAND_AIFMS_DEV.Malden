...
[_display, 1017, str (round _vehicleDirMils)] call itc_land_tablet_utils_fnc_setText;
[_display, 1018, str round (_turretDirMils + 3200)] call itc_land_tablet_utils_fnc_setText;
...
[_display, 1019, str round _realElevation] call itc_land_tablet_utils_fnc_setText;

_ammo = (configFile >> "CfgMagazines" >> _magazine >> "displayNameShort")  call BIS_fnc_getCfgData;

[_display, 1021, _ammo] call itc_land_tablet_utils_fnc_setText;

[_display, 1025, str _ammoCount] call itc_land_tablet_utils_fnc_setText;
...
if(count _solutions > 0) then {
  ...
  _solutionString = _solutionString + format ["QUADRANT %1<br/>", _qd];
  _solutionString = _solutionString + format ["TOF %1<br/>", _tof];
  _solutionString = _solutionString + format ["MAXIMUM ORDINATE %1<br/>", _maxOrd];
  _solutionString = _solutionString + format ["IMPACT ANGLE %1<br/>", _impAng];
  _solutionString = _solutionString + format ["TARGET DISTANCE %1<br/>", _dist];
