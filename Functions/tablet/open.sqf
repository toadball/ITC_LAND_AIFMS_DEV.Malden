...
[{
  ...
  if(_vehicle getVariable "app" != _app) then { //check if app switched
    ...
    _page = [_display] call call compile format["itc_land_tablet_fnc_%1_init", _app]; //initialize the new app
    _vehicle setVariable ["page", _page];
  };
  [_display] call call compile format["itc_land_tablet_fnc_%1_%2_render", _app, _page]; //render the app
  ...
  (_this select 0) set [3, _app];
  (_this select 0) set [4, _page];
